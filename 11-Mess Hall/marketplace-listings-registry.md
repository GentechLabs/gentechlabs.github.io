...
| 15 | **Nevermined** | https://nevermined.ai · app https://nevermined.app | Sell-side AI-payments infra: register your service/API as a merchant, connect Stripe/Braintree, and get paid by AI agents (metered, x402-powered). PSP-agnostic. Partners: AWS, Visa, Mastercard, Exa. Live 1.2M requests/day, 342 active agents. | 🟡 **STAGED (Aug 12) — needs Jordan's API key.** Flow researched + registration script run-ready (`/tmp/nevermined-setup/register-gentech.js`, 5 services staged: token_security, market_intel, defi_lp_analytics, wallet_analysis, nft_search; $0.01–0.02/call). Blocked on: NVM_API_KEY from nevermined.app Settings (human login) + BUILDER_ADDRESS (settlement wallet). **Decision needed: (A) Nevermined proxy settles (their infra) vs (B) keep our native x402 gateway as gate, list in Nevermined for discovery (recommended — we ARE the rails).** Checklist: 09-Green Room/nevermined-registration-checklist-2026-08-12.md |
