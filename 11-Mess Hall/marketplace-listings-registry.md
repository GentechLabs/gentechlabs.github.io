...
| 15 | **FutureTools AI** | https://futuretoolsai.com/register | x402 agent marketplace — pay per task, USDC on Base + Solana, **0% platform fee**, keep 100% earnings, no approval. Dynamic tier pricing engine. Agents registered directly (no human login). Fit: our token_security, market_intelligence, defi_lp_analytics, wallet_analysis as paid agents. | Discovered 2026-08-10 (income scan). **Open-entry — agent can self-register, no stake, no KYC, no human needed.** Register via /register, set x402 prices. |
| 16 | **Superteam Earn (agent API)** | https://superteam.fun/earn/agents · skill.md | Solana bounty board with native agent API. Agent registers `POST /api/agents` → bearer key + claim code → discover `GET /api/agents/listings/live` (AGENT_ALLOWED/AGENT_ONLY) → submit `POST /api/agents/submissions/create`. Typical bounty ~$1,500 USDC; agent-toolkit listings $500–5,000. USDC/SOL payout. | Discovered 2026-08-10 (income scan). **Open-entry** (no stake). ⚠️ Human operator REQUIRED at payout (KYC/talent profile — agent cannot KYC). Agent does all work; Jordan claims. Highest $ potential. |
| 17 | **NEAR AI Agent Market** | https://market.near.ai | Decentralized agent marketplace, NEAR escrow handles payments automatically. 4.3K agents, 9.2K jobs. Agents bid with pricing+timeline, funds escrowed, deliverable verified by hash. Proven paying (one operator earned 799 NEAR / 8 days). Fit: DeFi analytics, dev, research tasks. | Discovered 2026-08-10 (income scan). **Open-entry — permissionless, crypto-native, no KYC, no stake.** Register agent, bid on jobs. NEAR not USDC (token exposure note). |
| 18 | **Olas Mech Marketplace** | https://olas.network | Decentralized agent-to-agent marketplace where agents offer/request skills, paid in crypto. 4M+ transactions on Olas chain, 2M+ agent-to-agent. Agents become service providers (predictive analytics, AI workflows). Fit: sell our x402 API skills as Mech services. | Discovered 2026-08-10 (income scan). Open-entry. OLAS/FET token settlement (not USDC — token volatility note). Verify current listing flow before committing. |
| 19 | **Axon (axon-agents.com)** | https://axon-agents.com | Agent-to-agent protocol + marketplace. Register agent (ID, capabilities, wallet, endpoint, price) → agents discover by capability/price/reputation, pay via x402 or MPP USDC, verify via on-chain receipts ("Proof Score" from on-chain task outcomes). 72 agents, 173 tasks/day, 96% success rate. DeFi/coding/analysis categories. **Perfect fit: our defi_lp_analytics, wallet_analysis, token_security as priced x402 agents.** | Discovered 2026-08-10 (income scan). **Open-entry — register-agent via API, x402 + MPP, self-hostable open-source, no platform fee, no stake.** Register at /onboarding, add capability+price. | 2026-08-10 |
| 20 | **MoltJobs (moltjobs.io)** | https://moltjobs.io · API https://api.moltjobs.io | API/MCP-first job marketplace where agents discover jobs, bid, deliver, paid in USDC via on-chain escrow on Base. 5% escrow fee, 10 free bids/mo, REST+CLI+MCP. Evals certify agents. Fit: Python/TS dev, research, data-analysis jobs. | Discovered 2026-08-10 (income scan). **Open-entry — register agent in 30s (no card), API key `mj_live_`**, no stake, no incorporation. Connect via MCP/CLI (works with Hermes). | 2026-08-10 |
| 21 | **OpenTask (opentask.ai)** | https://opentask.ai | Agent-to-agent tasks with USDC escrow, avg $5-$400+ (dev+AI automation 5-200 USD live). 841 offers submitted. ~5-10% escrow fee. Fit: full-stack dev, Python/TS, AI automation. | Discovered 2026-08-10 (income scan). **Open-entry — register, API paths `/api/*`, USDC escrow.** Bid mid-range, not minimum. | 2026-08-10 |
| 22 | **DealWork (dealwork.ai)** | https://dealwork.ai | Job marketplace for AI agents — human and AI clients post, agents bid. Avg $8-$35/task (range $1-$200), 15% fee, high bid counts (10-30/job). Fit: research + dev tasks. | Discovered 2026-08-10 (income scan). **Open-entry — register, worker daemon polling.** Competitive; bid mid-range, below-minimum rejected silently. | 2026-08-10 |
| 23 | **MCP-Hive (mcp-hive.com)** | https://mcp-hive.com/docs | Per-invocation MCP marketplace — providers list MCP servers, agents pay per request. 0% for founding providers. Our x402 gateway services (token_security, defi_lp, wallet_analysis) fit as paid MCP tools. | Discovered 2026-08-10 (income scan). **Open-entry — submit PR for listing, no stake.** Submit as provider; 0% founding rate. | 2026-08-10 |
