...
- **OKX audit — 4 agents rejected, 3 root causes (Aug 22):** daemon healthy (ready:true, 4/4, CLI current). All 4 agents (#4905 Forge, #2849 DeFi, #2848 Curve, #2847 Treasury) have **empty `serviceList: []`** (→ A2MCP 404/405) + #2849 has **payment chain wrong (Base 8453 vs X Layer 196)**. **✅ FIXED + RELISTED (Aug 22, Jordan approved both):** (1) reordered `X402_NETWORKS` → xlayer first (verified challenge advertises eip155:196 first), (2) updated all 3 treasury agents' services to live `/v1/` endpoints (LP shape, market intel, token security, wallet portfolio), (3) deactivated #4905 Forge (dev agent, not treasury), (4) re-listed #2847/#2848/#2849 — **all 3 now "Listing under review"**. Forge stays deactivated. Watch for approval (24-48h).
