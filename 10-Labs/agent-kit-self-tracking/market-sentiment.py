#!/usr/bin/env python3
"""
Market Sentiment — the bull/bear sentiment radar (weekly).

Renamed from Narrative Rotation (Aug 21 2026). The small part was routing which
sector money is rotating to (compute, gold, games). The real capability: a
sentiment sensor that tells you the market SENTIMENT — are we bullish or
bearish? In a bear tape, "don't touch it." In a bull tape, "money's moving to X,
get in." Crossed with BTC dominance + stablecoin supply (the macro thermometer)
for a risk-on/risk-off stance.

Jordan: "not only know who's rotating where, but 'okay we're bearish right now,
don't touch it.'"
"""
...
def main():
    print("🔄 MARKET SENTIMENT — the bull/bear radar")
    print(f"{'='*55}")
    ...
    print(f"\n{'='*55}")
    print("📊 Source: CoinMarketCap + CoinGecko | Market Sentiment")
    print(f"{'='*55}")
    ...
    json_path = "/root/repos/ProtoJay4789.github.io/DeFi/rainbow/market-sentiment.json"
    try:
