...
def normalize_shape(shape: str) -> str:
    ...
    aliases = {
        "bidask": "bid-ask",        # bid-ask is distinct from bidirectional
        "bid-ask": "bid-ask",       # bid-ask is distinct from bidirectional  
        "bidirectional": "bidirectional",  # center-concentrated curve
        "curve": "curve",           # center-concentrated curve
        "spot": "spot",             # single price point
...
def calc_fee_efficiency(price: float, range_low: float, range_high: float, shape: str) -> float:
    ...
    if shape == "spot":
        ...
        # Bid-ask: concentrated on both immediate sides of current price
        # Efficiency peaks at ~75% when near edges (not 100%)
        # Center is ~50% (trading split between sides)
        center_dist = abs(pos - 0.5) * 2  # 0 at center, 1 at edges
        # 50% at center, ~75% at edges
        return round(50 + (center_dist * 25), 1)
    elif shape == "bidirectional":
        # Bidirectional: center-concentrated curve that tapers off on both sides
        return round(max(0, min(100, abs(pos - 0.5) * 2 * 100)), 1)
    else:
        # curve (standard center-concentrated)
...
def should_send_report(state: dict, price: float, efficiency: float, in_range: bool, alerts: list) -> tuple[bool, str]:
    ...
    if last_report is None:
        # Mid-efficiency stability: skip even baseline report (>=50% = earning good fees)
        if efficiency >= 50.0 and in_range and not alerts:
            state["consecutive_quiet_runs"] = 2   # mark quiet for the hour
            state["last_report_hour"] = now_dt.hour
            return False, "fee efficiency ≥50% — silent (earning good fees)"
        state["consecutive_quiet_runs"] = 0
        state["last_report_hour"] = now_dt.hour
    ...
    # No material change — mid-efficiency stability override?
    # When efficiency ≥50%, in range, and no alerts, stay silent all hour
    if efficiency >= 50.0 and in_range and not alerts:
