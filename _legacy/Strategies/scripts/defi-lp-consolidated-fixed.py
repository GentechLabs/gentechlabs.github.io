...
def format_report(price: float, in_range: bool, efficiency: float, pool_data: dict,
    ...
    if entry_price != "?" and float(entry_price) > 0:
        # Proper concentrated liquidity impermanent loss calculation        entry = float(entry_price)        if entry > 0:            ratio = price / entry            il_factor = (2 * ratio**0.5) / (1 + ratio)            il_pct = (il_factor - 1) * 100        else:            il_pct = 0.0
